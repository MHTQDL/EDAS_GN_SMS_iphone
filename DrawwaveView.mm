...
//	TRACE("draw rect\n");
    // Drawing code. 设置UIGraphicsGetCurrentContext上下文
...
//    }
//    if(m_bclear){
//        printf("TRUE-clear the screen ");
//    }else{
//        printf("FALSE- not clear");
//    }
//    if(siteDoc.m_pchannels==NULL){
//        printf("siteDoc.m_pchannels==NULL");
//    }
...
//    NSLog(@"DrawWave");
	...
	while(pt!=NULL)
	{
		if(pt->bShow)
		{
			...
			if(m_redraw_status==true && pt->position< pt->max_posi -10)
			{
                //开始画线
...
//    NSLog(@"OnData");
	...
	for(int j=0;j<chn_sum;j++)
	{
		...
		if(l==0){
           // 第一个通道：显示最新数据时间
            const time_t time=(siteDoc.m_pcur_chn->revblk.p_dsp->time);
			struct tm *ctm=localtime(&time);
