...
- (void)viewDidLoad
{
    ...
    //--------------------------------------------
    UILabel *labSens15 = [[UILabel alloc] initWithFrame : CGRectMake ( 20+xW , 260 +yH, 180 , 30 )];
    labSens15.text = @"方位角：";
    [scrollView addSubview:labSens15];[labSens15 release];
    txtAzimuthAngle = [[UITextField alloc] initWithFrame:CGRectMake(150+xW, 260+yH, 200, 30)];
    txtAzimuthAngle.borderStyle =  UITextBorderStyleRoundedRect;
    txtAzimuthAngle.delegate =self;
    [scrollView addSubview:txtAzimuthAngle];
	
	
	//-------------------------------------------- 
	UILabel *labSens7 = [[UILabel alloc] initWithFrame : CGRectMake ( 20+xW , 290 +yH, 180 , 30 )];
	labSens7.text = @"序列号：";
	[scrollView addSubview:labSens7];[labSens7 release];

	txtSerial = [[UITextField alloc] initWithFrame:CGRectMake(150+xW, 290+yH, 200, 30)];
	...
	UILabel *labSens8 = [[UILabel alloc] initWithFrame : CGRectMake ( 20+xW , 320 +yH, 180 , 30 )];
	labSens8.text = @"供应商：";
	[scrollView addSubview:labSens8];[labSens8 release];
	
	txtProvider = [[UITextField alloc] initWithFrame:CGRectMake(150+xW, 320+yH, 200, 30)];
	...
	UILabel *labSens9 = [[UILabel alloc] initWithFrame : CGRectMake ( 20+xW , 350+yH , 180 , 30 )];
	labSens9.text = @"分向信息";
	[scrollView addSubview:labSens9];[labSens9 release];
	
	UILabel *labSens10 = [[UILabel alloc] initWithFrame : CGRectMake ( 20+xW , 380+yH , 180 , 30 )];
	labSens10.text = @"分向数：";
	[scrollView addSubview:labSens10];[labSens10 release];
	
	
	txtComp = [[UITextField alloc] initWithFrame:CGRectMake(150+xW, 380+yH, 200, 30)];
	...
	//-------------------------------------------- 
	UILabel *labSens11 = [[UILabel alloc] initWithFrame : CGRectMake ( 20+xW , 410+yH , 180 , 30 )];
	labSens11.text = @"起始通道：";
	...
	cmboChn1AD = [[ UIComboBox alloc ] initWithFrame : CGRectMake ( 150+xW , 410+yH , 200 , 28 )];
	...
	//-------------------------------------------- 
	UILabel *labSens12 = [[UILabel alloc] initWithFrame : CGRectMake ( 20+xW , 440+yH , 180 , 30 )];
	labSens12.text = @"分向 1 测项分量代码：";
	[scrollView addSubview:labSens12];[labSens12 release];
	
	cmboChn1cxid = [[ UIComboBox alloc ] initWithFrame : CGRectMake ( 200+xW , 453+yH , 170 , 28 )];
    cmboChn1cxid.borderColor=[UIColor blueColor];
	[scrollView addSubview:cmboChn1cxid];[cmboChn1cxid release];
	
	//-------------------------------------------- 
	UILabel *labSens13 = [[UILabel alloc] initWithFrame : CGRectMake ( 20 +xW, 470+yH , 190 , 30 )];
	labSens13.text = @"分向 2 测项分量代码：";
	[scrollView addSubview:labSens13];[labSens13 release];
	
	cmboChn2cxid = [[ UIComboBox alloc ] initWithFrame : CGRectMake ( 200 +xW, 483+yH , 170 , 28 )];
    cmboChn2cxid.borderColor=[UIColor blueColor];
	[scrollView addSubview:cmboChn2cxid];[cmboChn2cxid release];
    
	//-------------------------------------------- 
	UILabel *labSens14 = [[UILabel alloc] initWithFrame : CGRectMake ( 20+xW , 500+yH , 180 , 30 )];
	labSens14.text = @"分向 3 测项分量代码：";
	[scrollView addSubview:labSens14];[labSens14 release];
	cmboChn3cxid = [[ UIComboBox alloc ] initWithFrame : CGRectMake ( 200+xW , 513+yH , 170 , 28 )];
    cmboChn3cxid.borderColor=[UIColor blueColor];
...
//	char temp[1024];
	...
	if(intcomp <1 || intcomp > 3) {
        [self showValidErro:@"分向数不能小于1和大于3"];
        return;}
    ...
    CM_SENSPARFRM frm;
    memset(&frm,0,sizeof(CM_SENSPARFRM));
//	char m_cmd[1024];
    char m_cmd[sizeof(CM_SENSPARFRM)+4];
	m_cmd[0]=0xbf;
	m_cmd[1]=0x13;
	m_cmd[2]=0x97;
	m_cmd[3]=0x74;
...
//	frm.head.length=212;
    ...
    sprintf(frm.state,"%s",[txtAzimuthAngle.text UTF8String]);
	frm.comp=intcomp;
...
//	sprintf(frm.state,"GC");
	...
	theApp.GetCXSID((int)[cmboType getSelectIndex],[[cmboChn1cxid.items objectAtIndex:[cmboChn1cxid getSelectIndex]] UTF8String], s);
	sprintf(frm.cxid[0],"%s",s.c_str());
	theApp.GetCXSID((int)[cmboType getSelectIndex],[[cmboChn2cxid.items objectAtIndex:[cmboChn2cxid getSelectIndex]] UTF8String], s);
	sprintf(frm.cxid[1],"%s",s.c_str());
	theApp.GetCXSID((int)[cmboType getSelectIndex],[[cmboChn3cxid.items objectAtIndex:[cmboChn3cxid getSelectIndex]] UTF8String], s);
 	sprintf(frm.cxid[2],"%s",s.c_str());
	
	short *p=(short *)&frm;
	frm.chk_sum=0;
    NSLog(@"frm： %lu",sizeof(frm));
//	for(int i=0;i<(frm.head.length+6)/2-1;i++)
    for(int i=0;i<sizeof(frm)/2-2;i++){
        NSLog(@"p1 值： %d",p[i]);
        frm.chk_sum-=p[i];
        NSLog(@"frm.chk_sum 值： %d  ",frm.chk_sum);
    }
    
	memcpy(&m_cmd[4],(char *)&frm,sizeof(CM_SENSPARFRM));
