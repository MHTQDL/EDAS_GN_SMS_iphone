...
int CSteim2 :: ZipASeries(long *data_in,char *data_out,int samp)
{
	...
	rec_len = 0;
	k       = 1;
	subFra.data[0].d32b = (int)data_in[0];
	for(i=0;i<samp;) {
		max =(int) dif[i];
		min = (int)dif[i];
		if(dif[i+1] > max) max =(int) dif[i+1];
		if(dif[i+1] < min) min =(int) dif[i+1];
		...
		if(dif[i+2] > max) max =(int) dif[i+2];
		if(dif[i+2] < min) min =(int) dif[i+2];
		if(max > 511 || min < -512) {
			SetIdBit(k,2);
		...
		if(dif[i+3] > max) max =(int) dif[i+3];
		if(dif[i+3] < min) min =(int) dif[i+3];
		...
		if(dif[i+4] > max) max = (int)dif[i+4];
		if(dif[i+4] < min) min =(int) dif[i+4];
		...
		if(dif[i+5] > max) max =(int) dif[i+5];
		if(dif[i+5] < min) min = (int)dif[i+5];
		...
		if(dif[i+6] > max) max = (int)dif[i+6];
		if(dif[i+6] < min) min = (int)dif[i+6];
