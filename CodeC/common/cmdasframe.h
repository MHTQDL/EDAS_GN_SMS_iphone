...
typedef struct tagCM_STNPARFRM
{
    ...
    short sens_sum;//地震计数
    short chn_sum;//通道数
    float lat;//纬度,单位：mas为(-90.0~90.0)*3600000
    float lon;//经度,单位：mas为(-180.0~180.0)*3600000
    float alt;//高程，单位：cm
...
typedef struct tagCM_SENSPARFRM
{
    ...
    char type[12];//标准类型,依照行业标准
    char bandstr[16];//带宽
    int depth;//埋深,cm   long depth;  需要4字节
    char rock[32];//岩基
    char state[4];//状态，GC   替换成 方位角
    char rspfile[32];//响应文件
    int comp;//分向数 long comp; long8字节   需要4字节
    int das_id[3];//分向对应的A/D通道序号，从1开始依次为高速A/D通道1-6，低速1-6
    char cxid[3][8];//依照DB/T3-2003 [地震及地震前兆测项分类及代码]标准
    short chk_sum;
}CM_SENSPARFRM;
...
typedef struct tagCM_STARTCALFRM
{
    ...
    int time;//--1:用于手工启动标定, 实际的启动时间由启动标定程序根据当前最新数据时间设置,>=0：用于定时启动标定，为有效的启动时间
    short chk_sum;
}CM_STARTCALFRM;
...
{
    CMDHEAD head;
    int baud;//波特率,4800,9600,19200,38400,57600,115200
    short chk_sum;//校验和
}CM_COMBAUDFRM;
