...
CSitemagDoc::CSitemagDoc()
{
	...
	m_bmsg_clr=TRUE;
	m_savefile_len=3600;
	m_savefile_begin = (int)time(NULL);
...
int CSitemagDoc::AllocRevblk(REVBLKS *prev,long samp)
{
	...
	for( i=0;i<60;i++){
		...
		prev->dat[i].time=-1;
		prev->dat[i].ds = (int *)(prev->p_dat1+i*size);
	}
	for(i=0;i<59;i++)
...
BOOL CSitemagDoc::ConnectSite()
{
    ...
    long m_begin_con1=(long)m_begin_con;
 	time(&m_begin_con1);
...
//2017-2-8 ProcessMessage方法没有被调用
//////////////////////////////¥¶¿Ì ’µΩµƒ–≈œ¢//////////////////////
...
//		}
        for(int i=0;i<frm.prd_num;i++){
            m_das.sens[id].sines.sine[i].cycle=frm.sine[i].cycle;
            m_das.sens[id].sines.sine[i].prd=frm.sine[i].prd;
            m_das.sens[id].sines.sine[i].aten=frm.sine[i].aten;
        }
	}
    if(frm.btimer==1){
		//fmt1.LoadString(IDS_ENABLESTR1);
...
void CSitemagDoc::OnBatcharge(char * pdata,string & info)
{
	...
	memcpy((char *)&frm,pdata,sizeof(frm));
	m_das.das.bat.time=(int)frm.time;
...
void CSitemagDoc::OnBatchargeStop(char * pdata,string & info)
{
	...
	char buff[1500];
	memcpy((char *)&frm,pdata,sizeof(frm));
	m_das.das.sbat.time=(int)frm.time;
...
void CSitemagDoc::OnHdStat(char * pdata,string & info)
{
	...
	if(frm.id==1 || frm.id==2){
		m_das.das.sd_stat[frm.id-1].total=(int)frm.total;//◊‹ø’º‰,µ•Œª:Kb
		m_das.das.sd_stat[frm.id-1].free=(int)frm.free;//ø…”√ø’º‰,µ•Œª:Kb
 		m_das.das.sd_stat[frm.id-1].be_curmass=frm.be_curmass;//1-’˝‘⁄–¥µƒ¥Ê¥¢ø®
		m_das.das.sd_stat[frm.id-1].io_error=(int)frm.io_error;//◊Ó∫Û1¥ŒI/O¥ÌŒÛ
...
void CSitemagDoc::OnSysTm(char * pdata,string & info)
{
    ...
    fmt = "\r\n当前系统时间：%d\r\n";
    char buff[35];
    sprintf(buff, fmt.c_str(), frm.sec);
	info = buff;
...
//网络授时服务器
void CSitemagDoc::OnNTP(char * pdata,string	& info)
{
    string fmt;
    CM_NTPSRVFRM frm;
    char buff[1500];
    ...
    memcpy((char *)&frm,pdata,sizeof(frm));
    fmt = "\r\n主服务器：%s\r\n备用服务器1：%s\r\n备用服务器2：%s\r\n";
    sprintf(buff, fmt.c_str(),frm.host1,frm.host2,frm.host3);
    info = buff;
    strcpy(m_das.das.ntpsrvfrm.host1,frm.host1);
    strcpy(m_das.das.ntpsrvfrm.host2,frm.host2);
    strcpy(m_das.das.ntpsrvfrm.host3,frm.host3);
...
void CSitemagDoc::OnInqNTPsrvfrm(){
    // TODO: Add your command handler code here
    SiteInquery(0x807c,0);
}
