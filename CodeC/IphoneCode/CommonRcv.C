...
BOOL CCommonRcv::ExpandCode(short *pexpand,short *pcompress,int cmd)
{
	...
	samp=(int)inbits.InputBits(12);
	c2index=(int)inbits.InputBits(12)+index;
	c3index=(int)inbits.InputBits(12)+index;
	if(cmd==0x55CC)
		num=samp/10;
	else{
		num=1;
...
void CCommonRcv::SetData(char *data,int * rcvdata,int comp,int wlen)
{
//    printf("SetData");
//    
