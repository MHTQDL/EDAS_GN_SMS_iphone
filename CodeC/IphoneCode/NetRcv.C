...
BOOL CNetRcv::ConnectToCmdServer()
{
    ...
    if(cmd!=100)
        return 0;
    sprintf(buf,"%s\r\n",m_connect.user);
    if(!m_sock.write_all(m_connect.cfd,buf, (int)strlen(buf))){
        return 0;
    }
...
int CNetRcv::Receive(int * prcv_data)
{
    short * p;
    int ret;
    p=(short *)m_dataframe.szData;
    
    char m_cmd[sizeof(m_dataframe.szData)];
//    printf(" ####Receive 接收 0x55dd: ");
//    if(p[1]==0x55dd){
...
void CNetRcv::Resend(int time,int length,int chn_id)
{
    char buf[200];
