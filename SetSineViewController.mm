...
- (void) saveFeqModeAction:(id)sender{
	int index = (int)[cmbFraModel getSelectIndex];
...
//        frm.sine[i].cycle=m_sine[i].cycle;
//        frm.sine[i].prd=m_sine[i].prd;
//        frm.sine[i].aten=m_sine[i].aten;
	}
	
	frm.chk_sum=0;
	p=(short *)&m_cmd[4];
    
    printf("for=%d \n",(frm.head.length+6)/2-2);
    for(i=0;i<(frm.head.length+6)/2-2;i++){
        frm.chk_sum-=p[i];
    }
	p[i]=frm.chk_sum;
